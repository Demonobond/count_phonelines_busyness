...
with open('/home/user/phones/sept.csv', 'r',encoding='cp1251') as f:
    reader = csv.reader(f,delimiter=';')
    next(reader) # Skip the header row.
    for row in reader:
        counter += 1
        #if counter>80 : break
        if row[7]=='Исходящий' and row[8]=='успешный':
            duration = sum(x * int(t) for x, t in zip([3600, 60, 1], row[9].split(":")))
            if duration == 0: continue 
            start=datetime.strptime(row[0]+" "+ row[1], '%Y-%m-%d %H:%M:%S')
            end=datetime.strptime(row[0]+" "+ row[1], '%Y-%m-%d %H:%M:%S')+timedelta(seconds=duration)
